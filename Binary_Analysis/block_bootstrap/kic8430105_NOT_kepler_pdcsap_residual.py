...
lc_model = np.loadtxt('work/model.PDCSAP.NOT')
lc_err = lc_model[:, 2]
residual = lc_model[:, 5]
rvA = np.loadtxt('work/rvA.NOT.dat')
rvB = np.loadtxt('work/rvB.NOT.dat')
rvA_model = np.loadtxt('work/rvA.NOT.pdcsap.model', unpack=True, usecols=4)
rvB_model = np.loadtxt('work/rvB.NOT.pdcsap.model', unpack=True, usecols=4)
param_names = [
...
n_cores = 10
infile_name = 'infile.residual.NOT.PDCSAP'
draw_random_rv_obs=False

# # Run with different block sizes
print('Residual block bootstrap with NOT RVs and Kepler PDCSAP LC, KIC 8430105.')
for n_blocks in n_blocks_list:
    ...
    np.savetxt(f'vals.NOT.PDCSAP.resi.{n_blocks}', vals)