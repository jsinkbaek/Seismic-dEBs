...
def residual_block_bootstrap(
        lc_model, residuals, error_residuals, rvA, rvB,
        repetitions, parameter_names, n_jobs=4,
        rvA_model=None, rvB_model=None, draw_random_rv_obs=False,
        infile_name='infile.default', split_function=None, split_args=None
):
    if split_function is None:
        model_blocks = np.copy(lc_model)
    elif callable(split_function) and split_args is not None:
        model_blocks = split_function(lc_model, *split_args)
    else:
        raise TypeError('split_function is not callable or None, or split_args is None.')
    clean_work_folder()
    job_results = Parallel(n_jobs=n_jobs)(
