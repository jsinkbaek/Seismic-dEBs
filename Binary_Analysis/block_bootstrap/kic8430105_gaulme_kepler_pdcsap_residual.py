...
lc_model = np.loadtxt('Data/model.PDCSAP.gaulme')
lc_err = lc_model[:, 2]
residual = lc_model[:, 5]
rvA = np.loadtxt('Data/rvA.gaulme.dat')
rvB = np.loadtxt('Data/rvB.gaulme.dat')
rvA_model = np.loadtxt('Data/rvA.gaulme.pdcsap.model', unpack=True, usecols=4)
rvB_model = np.loadtxt('Data/rvB.gaulme.pdscap.model', unpack=True, usecols=4)
param_names = [
...
for n_blocks in n_blocks_list:
    ...
    np.savetxt(f'Results/vals.gaulme.PDCSAP.resi.{n_blocks}', vals)