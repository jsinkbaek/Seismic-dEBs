...
nblocks = 7      # per eclipse

lc_model = np.loadtxt('work/model.KEPLER.gaulme')
lc_err = lc_model[:, 2]
time = lc_model[:, 0]
model = lc_model[:, 4]
residual = lc_model[:, 5]
...
mean_vals, std_vals, vals = boot.residual_block_bootstrap(
    ...
    infile_name='infile.residual.gaulme.KEPLER', draw_random_rv_obs=False
)
...
np.savetxt('vals.gaulme.KEPLER.seism.resi.7', vals)
