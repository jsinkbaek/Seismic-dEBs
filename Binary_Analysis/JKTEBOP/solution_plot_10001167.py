...
def lc_presentation(lc_, xlim1, xlim2, lc_exclusions=None):
    mag = lc_[:, 1]
    err = lc_[:, 2]
    phase = lc_[:, 3]
    model = lc_[:, 4]
    ...
    ax11.set_ylabel('Relative Magnitude')
    ax21.set_ylabel('O-C')
    ...
    ax21.xaxis.set_major_locator(plt.MaxNLocator(4, symmetric=True, min_n_ticks=4))
    ax22.xaxis.set_major_locator(plt.MaxNLocator(4))
    ax21.set_xticks([-0.02, 0, 0.02])
    ax22.set_xticks([0.645, 0.660, 0.675])
    plt.tight_layout()
...
def rv_plot_1(rv1_a, rv1_b, rv1_model, markersize=6, linewidth=2, elinewidth=1):
    fig = plt.figure(figsize=(11, 11))
    ...
    ax1.errorbar(
        rv1_a[:, 3], rv1_a[:, 1], yerr=rv1_a[:, 2], fmt='D', color='indianred', markersize=markersize, label='RG',
        elinewidth=elinewidth
    )
    ax1.errorbar(
        rv1_a[:, 3]-1, rv1_a[:, 1], yerr=rv1_a[:, 2], fmt='D', color='indianred', markersize=markersize,
        elinewidth=elinewidth
    )

    ax1.errorbar(
        rv1_b[:, 3], rv1_b[:, 1], yerr=rv1_b[:, 2], fmt='D', color='royalblue', markersize=markersize,
        label='MS', elinewidth=elinewidth
    )
    ax1.errorbar(
        rv1_b[:, 3] - 1, rv1_b[:, 1], yerr=rv1_b[:, 2], fmt='D', color='royalblue', markersize=markersize,
        elinewidth=elinewidth
    )

    ax1.plot(rv1_model[:, 0], rv1_model[:, 7], linestyle='dotted', alpha=0.8, color='royalblue', linewidth=linewidth)
    ax1.plot(rv1_model[:, 0] - 1, rv1_model[:, 7], linestyle='dotted', alpha=0.8, color='royalblue', linewidth=linewidth)
    ax1.plot(rv1_model[:, 0], rv1_model[:, 6], linestyle='dotted', alpha=0.8, color='indianred', linewidth=linewidth)
    ax1.plot(rv1_model[:, 0] - 1, rv1_model[:, 6], linestyle='dotted', alpha=0.8, color='indianred', linewidth=linewidth)
    ax1.set_ylim([-50, ax1.get_ylim()[1]])
    ax1.set_yticks([-40, -20, 0, 20, 40])
    ax1.legend(ncol=2)
    ...
    ax21.errorbar(
        rv1_a[:, 3], rv1_a[:, 5], yerr=rv1_a[:, 2], fmt='D', color='indianred', label=f'RMS: {rms1_a:.3f} km/s',
        markersize=markersize, elinewidth=elinewidth
    )
    ax21.errorbar(
        rv1_a[:, 3]-1, rv1_a[:, 5], yerr=rv1_a[:, 2], fmt='D', color='indianred', markersize=markersize,
        elinewidth=elinewidth
    )
    # ax21.legend(loc='upper left', fontsize=14)
    ...
    ax21.plot([-0.05, 1], [0, 0], '--', color='black', alpha=0.7)
    ax22.errorbar(
        rv1_b[:, 3], rv1_b[:, 5], yerr=rv1_b[:, 2], fmt='D', color='royalblue', label=f'RMS: {rms1_b:.3f} km/s',
        markersize=markersize, elinewidth=elinewidth
    )
    ax22.errorbar(
        rv1_b[:, 3]-1, rv1_b[:, 5], yerr=rv1_b[:, 2], fmt='D', color='royalblue', markersize=markersize,
        elinewidth=elinewidth
    )
    # ax22.legend(loc='upper left', fontsize=14)
